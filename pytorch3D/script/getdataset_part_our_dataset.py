...
for max_image in max_images:
    ...
    out_dir = f"cutting data {max_image} slices"


    add_img_count = max_image-last_point
    last_point = max_image
    for i in range(len(size_data_arr)):
        ...
        iter_work_image_names = random.sample(image_names, add_img_count)
        
        for name in iter_work_image_names:
            test_num_img_name = len(image_names) 
            image_names.remove(name)
            assert test_num_img_name == len(image_names) + 1

        work_image_names += iter_work_image_names
      
        for img_name in work_image_names:
        ...
        for i,dir_name in enumerate(file_dir_arr):
            for mask_name in work_image_names:
