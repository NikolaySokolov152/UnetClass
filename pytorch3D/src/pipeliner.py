...
class Pipeliner:
    ...
    def train_block(self,
        ...
        for epoch_train_iteration, train_data in enumerate(tqdm_train_loop):
            ...
            loss = self.lossesFunction(outputs, targets, batch_statistic)
        ...
        if use_train_metric:
            for i, metric_name in enumerate(self.metric_names_list):
                metrics_view_epoch[metric_name] = np.round(train_metric[i].numpy() / len(tqdm_train_loop),4)
    ...
    def save_pipeliner(self):
        ...
        self.load_model_weights(self.temp_best_model_weights) # установить веса с лучшим значением
    ...
    def train(self,
        ...
        for epoch in range(num_epoch):
            ...
            #tqdm_train_loop=tqdm(my_data_generator.getTrainDataLoaderPytorch(0),
                                   desc="\t",
                                   ncols=cmd_size-len(f"lr= {now_lr}")-3 if cmd_size else None,
                                   file=sys.stdout,
                                   colour="GREEN",
