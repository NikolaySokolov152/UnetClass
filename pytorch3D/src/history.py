class History:
    ...
    def update_train_metrics(self, metric_names, value_metrics):
        for i, metric in enumerate(metric_names):
            self.history["metrics"]["train_metrics"][metric].append(float(value_metrics[i]))
    def update_val_metrics(self, metric_names, value_metrics):
        for i, metric in enumerate(metric_names):
            self.history["metrics"]["val_metrics"][metric].append(float(value_metrics[i]))

    def update_train_metric(self, metric_name, value_metric):
        self.history["metrics"]["train_metrics"][metric_name].append(float(value_metric))
    def update_val_metric(self, metric_name, value_metric):
        self.history["metrics"]["val_metrics"][metric_name].append(float(value_metric))
