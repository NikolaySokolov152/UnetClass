...
from keras.preprocessing import image as krs_prep_img
...
class Generator(keras.utils.all_utils.Sequence):
        def __init__(self, dir_data, list_class_name, num_classes, transform_data, aug_dict, mode, save_inform, tailing, augment):
        ...
        def create_transform(self, aug_dict):
            ...
            if self.augment:
                ...
                list_compose.append(albu.ShiftScaleRotate(p=0.5, rotate_limit = 0, scale_limit = 0, shift_limit_x = aug_dict["width_shift_range"],\
                ...
                list_compose.append(albu.RandomResizedCrop(p=1, height = self.transform_data.target_size[1], width = self.transform_data.target_size[0],\
                                                           scale = (1 - aug_dict["zoom_range"], 1 + aug_dict["zoom_range"]),
                                                           ratio = (1 - aug_dict["zoom_range"], 1 + aug_dict["zoom_range"])))
                                                      
            list_compose.append(albu.Resize(height = self.transform_data.target_size[1], width = self.transform_data.target_size[0]))
        ...
        def generate_X(self, list_name_in_batch):
            ...
            for i, name in enumerate(list_name_in_batch):
                ...
                if self.transform_data.color_mode_img == "rgb":                    
                    image = krs_prep_img.load_img(img_path)
                    img = krs_prep_img.img_to_array(image)
                    ...
                    image = krs_prep_img.load_img(img_path, color_mode = "grayscale")
                    img = krs_prep_img.img_to_array(image)
            ...
            if self.transform_data.mode_mask == 'separated':
                for j, name in enumerate(list_name_in_batch):
                    ...
                    for i in range(self.num_classes):
                        img_path = os.path.join(self.dir_data.dir_mask_name, self.list_class_name[i] , self.dir_data.add_mask_prefix + name)

                        if os.path.isfile(img_path):
                            image = krs_prep_img.load_img(img_path, color_mode = "grayscale")
                            masks = krs_prep_img.img_to_array(image)[:,:,0]
...
class DataGenerator():
    ...
    def __init__(self, dir_data = InfoDirData(),
        ...
        print("In normal dir:", len(self.list_img_name), "images")

        random.shuffle(self.list_img_name)
       
        if self.mode == "train":
            self.gen_train = Generator(dir_data, list_class_name, num_classes, transform_data, aug_dict, mode, [save_inform,"train generator"], tailing, augment)
            self.gen_valid = Generator(dir_data, list_class_name, num_classes, transform_data, aug_dict, mode, [save_inform,"validation generator"], tailing, augment)
       
        else:    
            self.gen_test = Generator(dir_data, list_class_name, num_classes, transform_data, aug_dict, mode, [save_inform,"test generator"], tailing, False)
    ...
    def load_new_data(self, img_names):
        ...
        for name in img_names:
            ...
            if self.transform_data.color_mode_img == "rgb":                    
                image = krs_prep_img.load_img(img_path)
                img = krs_prep_img.img_to_array(image)
                img = krs_prep_img.smart_resize(img, self.transform_data.target_size)
                #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            else:
                image = krs_prep_img.load_img(img_path, color_mode = "grayscale")
                img = krs_prep_img.img_to_array(image)
        ...
        if self.transform_data.mode_mask == 'separated':
            for name in img_names:
                ...
                for i in range(self.num_classes):
                    ...
                    if os.path.isfile(img_path):
                        image = krs_prep_img.load_img(img_path, color_mode = "grayscale")
                        masks = krs_prep_img.img_to_array(image)
                        masks = krs_prep_img.smart_resize(masks, self.transform_data.target_size)[:,:,0]
                        masks = masks.astype(np.float32)
                        masks = to_0_1_format_img(masks)
                    else:
                        #print("no open ", img_path)
        ...
        if self.shuffle == True:
            ...
            if self.subsampling == 'random':
                ...
                self.list_img_name = list_shuffle_test + list_shuffle_train
                 ...
                 self.list_img_name = self.list_img_name[len(self.list_img_name) - size_val:] + self.list_img_name[:len(self.list_img_name) - size_val]
...
class AugmentGenerator(keras.utils.all_utils.Sequence):
...
class DataGeneratorReaderAll():
    ...
    def __init__(self, dir_data = InfoDirData(),
        ...
        if self.mode == "train":
            self.gen_train = AugmentGenerator(self.all_imges, self.all_masks, transform_data, aug_dict, mode, [save_inform,"train generator"], tailing, augment)
            self.gen_valid = AugmentGenerator(self.all_imges, self.all_masks, transform_data, aug_dict, mode, [save_inform,"validation generator"], tailing, augment)
     
        else:    
            self.gen_test = AugmentGenerator(self.all_imges, self.all_masks, transform_data, aug_dict, mode, [save_inform,"train generator"], tailing, augment)
    ...
    def load_new_data(self, img_names):
        ...
        bar = IncrementalBar('Load slices', max = len(img_names))
        ...
        for name in img_names:
            ...
            if self.transform_data.color_mode_img == "rgb":                    
                ...
                image = krs_prep_img.load_img(img_path, color_mode = "grayscale")
                img = krs_prep_img.img_to_array(image)
                #img = krs_prep_img.smart_resize(img)
        ...
        bar.finish()
        bar = IncrementalBar('Load masks', max = len(img_names))

        if self.transform_data.mode_mask == 'separated':
            for index_name, name in enumerate(img_names):
                bar.next()
